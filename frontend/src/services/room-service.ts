...
export const updateRoomWithChecklist = async (
  ...
  await api.put(`/api/v1/rooms/${roomId}`, {
    name: basic.name,
    rentType: RENT_TYPE_MAP[basic.transactionType ?? ''] ?? 'MONTHLY',
    isManagementFeeUnknown: basic.isMgmtUnknown,
    hasLoan: basic.loanStatus === '있음',
    canRegisterAddress: basic.moveInReport === '가능',
    buildingType: BUILDING_TYPE_MAP[building.buildingType ?? ''] ?? 'ONE_ROOM',
    hasElevator: building.elevator === '있음',
    direction: DIRECTION_MAP[building.direction ?? ''] ?? 'SOUTH',
    ...
    isMoveInDateNegotiable: basic.moveInNegotiable,
